...
ffbuild_dockerbuild() {
    ...
    cmake -DCMAKE_TOOLCHAIN_FILE="$FFBUILD_CMAKE_TOOLCHAIN" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$FFBUILD_PREFIX" -DWITH_OPENMP=ON -DBUILD_{TESTS,EXAMPLES,SHARED_LIBS}=OFF ..
    make -j$(nproc)
    make install
